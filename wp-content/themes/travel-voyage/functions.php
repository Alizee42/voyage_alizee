...
function travel_voyage_customizer_register( $wp_customize ){
    ...
    $wp_customize->add_control(
        new Chic_Lite_Toggle_Control( 
            ...
            array(
                'section'       => 'typography_settings',
                'label'         => __( 'Load Google Fonts Locally', 'travel-voyage' ),
                'description'   => __( 'Enable to load google fonts from your own server instead from google\'s CDN. This solves privacy concerns with Google\'s CDN and their sometimes less-than-transparent policies.', 'travel-voyage' )
            )
        )
    );   
    ...
    $wp_customize->add_control(
        new Chic_Lite_Toggle_Control( 
            ...
            array(
                'section'       => 'typography_settings',
                'label'         => __( 'Preload Local Fonts', 'travel-voyage' ),
                'description'   => __( 'Preloading Google fonts will speed up your website speed.', 'travel-voyage' ),
    ...
    ob_start(); ?>
        ...
        <span style="margin-bottom: 5px;display: block;"><?php esc_html_e( 'Click the button to reset the local fonts cache', 'travel-voyage' ); ?></span>
        
        <input type="button" class="button button-primary chic-lite-flush-local-fonts-button" name="chic-lite-flush-local-fonts-button" value="<?php esc_attr_e( 'Flush Local Font Files', 'travel-voyage' ); ?>" />
    <?php
    $travel_voyage_flush_button = ob_get_clean();
    ...
    $wp_customize->add_control(
        'ed_flush_local_fonts',
        array(
            'label'         => __( 'Flush Local Fonts Cache', 'travel-voyage' ),
            'section'       => 'typography_settings',
            'description'   => $travel_voyage_flush_button,
            'type'          => 'hidden',
            'active_callback' => 'chic_lite_ed_localgoogle_fonts'
        )
    );
